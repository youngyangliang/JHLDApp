...
@interface ChangeTelephoneController ()<UITextFieldDelegate>
...
@implementation ChangeTelephoneController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"修改手机号";
    [self setUpUI];
...
-(void)changeButtonClick{
    if ([self verify]) {
        ...
        [param setValue:self.userNameField.text forKey:@"nUserLogin"];
        [param setValue:self.codeField.text forKey:@"pin"];
        [RequestData POST:@"updateLogin" parameters:param response:^(id responseObject, BOOL responseOK, NSString *msg) {
            if (responseOK) {
                [ProgressHUD showSuccess:@"修改成功"];
                [Helper updataUserDefultValue:self.userNameField.text forKey:@"name"];
